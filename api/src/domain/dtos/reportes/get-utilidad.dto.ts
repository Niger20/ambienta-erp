export class GetUtilidadDiariaDto {
    ...
    static create(props: { [key: string]: any }): [string?, GetUtilidadDiariaDto?] {
        ...
        let start = new Date(`${todayStr}T00:00:00.000${GetUtilidadDiariaDto.TZ_OFFSET}`);
        let end = new Date(`${todayStr}T23:59:59.999${GetUtilidadDiariaDto.TZ_OFFSET}`);
        ...
        if (fechaInicio) {
            ...
            const adjustedStr = dateStr.includes('T') ? dateStr : `${dateStr}T00:00:00.000${GetUtilidadDiariaDto.TZ_OFFSET}`;
        ...
        if (fechaFin) {
            ...
            const adjustedStr = dateStr.includes('T') ? dateStr : `${dateStr}T23:59:59.999${GetUtilidadDiariaDto.TZ_OFFSET}`;
        ...
        return [undefined, new GetUtilidadDiariaDto(start, end)];
    }
}
