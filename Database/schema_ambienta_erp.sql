--
-- Ambienta ERP — Script SQL de creación de la base de datos (Etapa 1)
--
-- Generado con: npx prisma migrate diff --from-empty --to-schema prisma/schema.prisma --script
-- (a partir de api/prisma/schema.prisma) y validado corriendo limpio contra un Postgres 15 vacío.
--
-- Uso: pegar este script completo en una base de datos Neon (u otro Postgres) recién creada y vacía.
-- Crea las 61 tablas del sistema completo (POS/ERP + planillas) MÁS las nuevas tablas de esta etapa:
-- roles, permisos, rolespermisos, verificacionestoken, tokensesion, y las columnas nuevas de
-- perfil/roles en "usuarios" (nombre, apellido, correo, telefono, fotoperfil, correoverificado, rolid).
--
-- Después de correr este script, sembrar el catálogo de roles/permisos y el usuario administrador
-- por defecto con:
--   cd api
--   POSTGRES_URL="<tu-connection-string-de-neon>" npm run db:seed
-- (usuario/contraseña por defecto: admin / Admin123! — cámbiala después de iniciar sesión)
--
-- Nota sobre las columnas GENERATED ALWAYS AS (...) STORED: Prisma las introspectó desde la BD
-- original de Neon como columnas calculadas; este script las recrea con la sintaxis nativa de
-- Postgres para columnas generadas (no como un DEFAULT literal, que Postgres no permite si
-- referencia otras columnas).
--
